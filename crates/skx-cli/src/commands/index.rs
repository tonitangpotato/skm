...
pub struct IndexArgs {
    ...
    /// Output index file
    #[arg(short, long, default_value = ".skx-index.bin")]
    output: PathBuf,
...
pub async fn index(args: IndexArgs) -> anyhow::Result<()> {
    ...
    #[cfg(any(feature = "embed-bge-m3", feature = "embed-minilm"))]
    {
        ...
        use skx_embed::{ComponentWeights, EmbeddingIndex, EmbeddingProvider};
        ...
        let provider: Box<dyn EmbeddingProvider> = match args.model.as_str() {
            ...
            "bge-m3" | "bge" => {
                ...
                Box::new(skx_embed::BgeM3Provider::new()?)
            }
            #[cfg(feature = "embed-minilm")]
            "minilm" | "mini" => {
                println!("Loading MiniLM model...");
                Box::new(skx_embed::MiniLmProvider::new()?)
            }
            other => {
