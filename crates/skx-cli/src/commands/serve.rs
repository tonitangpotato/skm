...
async fn list_skills(registry: Arc<skx_core::SkillRegistry>) -> axum::Json<serde_json::Value> {
...
async fn select_skill(
    selector: Arc<skx_select::CascadeSelector>,
    registry: Arc<skx_core::SkillRegistry>,
    axum::Json(body): axum::Json<serde_json::Value>,
) -> axum::Json<serde_json::Value> {
    let query = body.get("query").and_then(|v| v.as_str()).unwrap_or("");
    let ctx = skx_select::SelectionContext::new();
