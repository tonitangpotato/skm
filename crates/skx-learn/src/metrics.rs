...
impl SelectionMetrics {
    ...
    pub fn to_prometheus(&self) -> String {
        ...
        for (conf, count) in &summary.by_confidence {
            lines.push(format!(
                "skx_selections_by_confidence{{confidence=\"{}\"}} {}",
                conf, count
            ));
        }
        ...
        for (strategy, count) in &summary.by_strategy {
            lines.push(format!(
                "skx_selections_by_strategy{{strategy=\"{}\"}} {}",
                strategy, count
            ));
        }
...
mod tests {
    ...
    fn test_prometheus_export() {
        ...
        assert!(prometheus.contains("skx_selections_total 1"));
        assert!(prometheus.contains("skx_selections_by_strategy{strategy=\"trigger\"}"));
    }
}
