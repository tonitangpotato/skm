    ...
    fn parse_response(
        ...
        let results: Vec<SelectionResult> = parsed
            ...
            .map(|s| {
                ...
                let skill_name = skx_core::SkillName::new(&s.name).unwrap_or_else(|_| {
                    skx_core::SkillName::new("unknown").unwrap()
                });
...
mod tests {
    ...
    fn make_metadata(name: &str, description: &str) -> SkillMetadata {
        SkillMetadata {
            name: skx_core::SkillName::new(name).unwrap(),
